...
---Vistas---
--1.	Vista para obtener el listado de todos los cursos:
...
--2.	Vista para obtener el listado de todos los cursos por nivel:

CREATE VIEW vw_cursos_por_nivel AS
SELECT IdCurso, Nombre, Nivel
FROM cursos
ORDER BY Nivel;

--3.	Vista para obtener el listado de todos los cursos por modalidad:

CREATE VIEW vw_cursos_por_modalidad AS
SELECT IdCurso, Nombre, Modalidad
FROM cursos
ORDER BY Modalidad;

--4.	Vista para obtener el listado de todos los cursos y sus horarios:

CREATE VIEW vw_cursos_y_horarios AS
SELECT cursos.IdCurso, cursos.Nombre, cursos.Nivel, cursos.Modalidad, horarios.DiaSemana, horarios.HoraInicio, horarios.HoraFin
FROM cursos
INNER JOIN horarios ON cursos.IdHorario = horarios.IdHorario;

--5.	Vista para obtener el listado de todos los cursos y la fecha de su última modificación:

CREATE VIEW vw_cursos_y_ultima_modificacion AS
SELECT cursos.IdCurso, cursos.Nombre, cursos.Nivel, cursos.Modalidad, MAX(cursos.FechaModificacion) AS FechaUltimaModificacion
FROM cursos
GROUP BY cursos.IdCurso, cursos.Nombre, cursos.Nivel, cursos.Modalidad;

--6.	Vista para obtener el listado de todos los cursos y la cantidad de veces que han sido modificados:

CREATE VIEW vw_cursos_y_cantidad_modificaciones AS
SELECT cursos.IdCurso, cursos.Nombre, cursos.Nivel, cursos.Modalidad, COUNT(DISTINCT cursos.IdModificacion) AS CantidadModificaciones
FROM cursos
GROUP BY cursos.IdCurso, cursos.Nombre, cursos.Nivel, cursos.Modalidad;

--7.	Vista para obtener el listado de todos los cursos y su promedio de modificaciones por mes:

CREATE VIEW vw_cursos_y_promedio_modificaciones_por_mes AS
SELECT cursos.IdCurso, cursos.Nombre, cursos.Nivel, cursos.Modalidad, 
    COUNT(DISTINCT cursos.IdModificacion) / 
    ROUND(MONTHS_BETWEEN(SYSDATE, cursos.FechaCreacion)) AS PromedioModificacionesPorMes
FROM cursos
GROUP BY cursos.IdCurso, cursos.Nombre, cursos.Nivel, cursos.Modalidad;

--8.	Vista para obtener el listado de todos los cursos que se han creado en los últimos 30 días:

CREATE VIEW vw_cursos_creados_ultimos_30_dias AS
SELECT *
FROM cursos
WHERE FechaCreacion >= SYSDATE - 30;

--9.	Vista para obtener el listado de todos los cursos que se han modificado en los últimos 30 días:

CREATE VIEW vw_cursos_modificados_ultimos_30_dias AS
SELECT *
FROM cursos
WHERE FechaModificacion >= SYSDATE - 30;

--10.	Vista para obtener el listado de todos los cursos por nivel y modalidad, y la cantidad de cursos por cada combinación:

CREATE VIEW vw_cursos_por_nivel_y_modalidad AS
SELECT Nivel, Modalidad, COUNT(*) AS CantidadCursos
FROM cursos
